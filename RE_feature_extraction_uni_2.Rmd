---
title: "Retention Project | Feature extraction uni_2"
output:
  html_document:
    code_folding: show
    toc: yes
...
library('knitr')
opts_knit$set(root.dir = normalizePath('../Data/Raw Data/uni_2'))
...
clicked_notifications_tab <- read_csv("clicked_notifications_tab.csv")
...
retention <- read_csv("user_retention_data_comb.csv")
```


# Functions
```{r functions}
# first overview of data strcture and check for missing data
check_df <- function(df) {
  head <- head(df)
  structure <- str(df)
  summary <- summary(df)
  colsums <- colSums(is.na(df))
  return(list(head, structure, summary, colsums))
}
...
## 2. Add column to df automatically, classifiying each row as "firstterm" and "not firstterm" to see whether the date_created lies in the respective first term of the student
mutate_var_in_firstterm <- function(df, date) {
  date <- enquo(date) #returns value as a quosure
 feat_df <- df %>%
  mutate(in_firstterm = case_when(term_of_admission == "Fall 2016" & !!date >= ymd(20160820) & !!date <= ymd(20161220) ~ "firstterm", #!! used to unquote an input so that it’s evaluated, not quoted
                                  term_of_admission == "Fall 2017" & !!date >= ymd(20170820) & !!date <= ymd(20171220) ~ "firstterm",
                                  term_of_admission == "Fall 2018" & !!date >= ymd(20180820) & !!date <= ymd(20181220) ~ "firstterm",
                                  term_of_admission == "Fall 2019" & !!date >= ymd(20190820) & !!date <= ymd(20191220) ~ "firstterm",
                                  TRUE ~ "not firstterm"))
...
# Extract slopes from linear regression to represent the change of a certain app use behavior over time
my.slopeExtraction <- function(df, date_var, feat_name) {
...
First, the institutional data set is cleaned.
We need the *term of admission for each student* to determine whether a log lies within the respective first term of the student. Therefore, we prepare a subset of the retention data that can easily be added to all data sets.

```{r preparation institutional data, include=FALSE}
#cleaning institutional data set
missmap(retention, main = "Missing values vs observed")

# rename levels with missspellings
retention <- retention %>%
     mutate(program_information = dplyr::recode(program_information, 
                                             'Business- Hospitality &amp; Event (BUS-HOS EVNT)' = "Business- Hospitality & Event (BUS-HOS EVNT)",
                                             'Secondary Ed &amp; K-12 Business (SEC-ED-BUS)' = "Secondary Ed & K-12 Business (SEC-ED-BUS)",
                                             'Interior Architecture &amp; Design (INT-ARCH-DES)' = "Interior Architecture & Design (INT-ARCH-DES)",
                                             'Music Therapy (MUSIC-THERAP)' = "Music Therapy (MUSIC-THERA)",
                                             'Comm Sciences &amp; Disorders (COMM SCI DIS)' = "Comm Sciences & Disorders (COMM SCI DIS)",
                                             'Horticulture (HORT_CULTURE)' = "Horticulture (HORTICULTURE)"))

# remove users for whom we do not have the term of admission
# remove users with user_id = 0, those did not use the app at all and could therefore not be matched to the app use data
retention_clean <- retention %>% filter(!is.na(term_of_admission)) %>% rename("program_status_credits" = "program_status_(credits)") %>% distinct() %>% filter(user_id != 0)
# remove rows with duplicate user_ids, they are enrolled in several programs
retention_clean <- retention_clean[!duplicated(retention_clean$user_id),]


# Creating a subset of retention df to have an overview of user_ids plus their terms of admission
...
# Feature Extraction
Features are extracted form the cleaned data sets. The goal is to result with a data set with one row for each student (user_id) and **all valuable information** in the columns. The information are stored in form of variables, so-called **features**, intended to be informative and non-redundant and describe the data with sufficient accuracy. The features will be used in the model to make predictions.
First, data sets will be prepared and features extracted from single data sets. Subsequently, composite features are created by joining different data sets or different feature sets.

## Institutional data
Overview and preparation:
```{r ret}
# replace ACT score values of 0 with NA, assuming that the student did not take the respective exam, because 0 is not possible for ACT
retention_clean <- replace_with_na_at(retention_clean,
                 "act_score",
                 condition = ~.x == 0)
...
Feature extraction:
Please note, that **not all variables from the institutional data are included as features**. We exclude variables that have a bias due to the lenght of the studies (e.g., term of admission, cumulative credits earned) as well as the study programm, as it cannot be generalized to other universities.
```{r feat ret}
feat_ret_f <- retention_clean %>%
  group_by(user_id) %>% 
  summarize(status,
            inst_student_degree_type = student_degree_type,
            inst_cumulative_gpa = cumulative_gpa,
            inst_act_score = act_score,
            inst_high_school_gpa = high_school_gpa,
            inst_gender = gender,
            inst_living_in_residence = living_in_residence_hall,
            inst_athlete = athlete,
            inst_pell_eligibile = pell_eligibile,
            inst_ethnicity = ethnicity)
```
...
# Create a joint data set for app_session and retention containing all user_ids
app_session_retention <- inner_join(retention_subset, app_session, by = 'user_id')
# Add column classifying each row into firstterm and not firstterm
app_session_first <- mutate_var_in_firstterm(app_session_retention, app_open_time)
```
Preparation of the data set:
```{r preparation app session, include=FALSE}
# Preparation of app session data set
app_session_first <- app_session_first %>%
  filter(app_open_time > ymd(20150101),
         app_close_time > ymd(20150101)) %>% # app was only launched in 2015, dates before 2015 are unvalid
  mutate(app_open_date = as.Date(app_open_time),
         app_close_date = as.Date(app_close_time),
         app_open_weekdays = wday(app_open_time, week_start = getOption("lubridate.week.start", 7)), #add column identifying the weekdays of use (0-7 starting on Sunday)
         sess_duration = as.numeric(difftime(app_close_time, app_open_time, units = "secs"))) %>% # calculate each session's duration
  mutate(sess_duration = replace(sess_duration, which(sess_duration < 0), NA)) %>% # replace negative session durations with NA as app_close_time was unvalid and we have no information about the actual duration
  mutate(app_before_firstterm = case_when( #create column that categorizes each session according to whether that session was before the respective student's first term start
    term_of_admission == "Fall 2016" & app_open_time < ymd(20160820) ~ "TRUE", #i.e. if a student was admitted in fall 2016 and used the app before the start of term 2016, he/she gets a TRUE
    term_of_admission == "Fall 2017" & app_open_time < ymd(20170820) ~ "TRUE",
    term_of_admission == "Fall 2018" & app_open_time < ymd(20180820) ~ "TRUE",
    term_of_admission == "Fall 2019" & app_open_time < ymd(20190820) ~ "TRUE",
    TRUE ~ "FALSE")) %>%
  mutate(app_first_winter = case_when( #create column that categorizes each session according to whether that session was during the respective student's first winter break
    term_of_admission == "Fall 2016" & app_open_time > ymd(20161220) & app_open_time < ymd(20170125) ~ "TRUE", #i.e. if a student was admitted in fall 2016 and used the app after end of the frist term and before the start of the second term, he/she gets a TRUE
    term_of_admission == "Fall 2017" & app_open_time > ymd(20171220) & app_open_time < ymd(20180125) ~ "TRUE",
    term_of_admission == "Fall 2018" & app_open_time > ymd(20181220) & app_open_time < ymd(20190125) ~ "TRUE",
    term_of_admission == "Fall 2019" & app_open_time > ymd(20191220) & app_open_time < ymd(20200125) ~ "TRUE",
    TRUE ~ "FALSE"))

app_session_first$app_before_firstterm <- as.factor(app_session_first$app_before_firstterm)
app_session_first$app_first_winter <- as.factor(app_session_first$app_first_winter)
```

Feature extraction:
```{r feature app session}
feat_app_sess_f <- app_session_first %>% 
  filter(in_firstterm == "firstterm") %>% #as in the df app_session_first all sessions are still included, we need to filter for the ones in the first term to get the measures only for that time
...
feat_app_sess_f2 <- app_session_first %>%
  group_by(user_id) %>%
  summarize(ae_app_before_firstterm = ifelse(TRUE %in% app_before_firstterm, TRUE, FALSE), #if a students has any app session before first term (any TRUE in the column `app_before_firstterm`) he/she gets a TRUE
            ae_app_first_winter = ifelse(TRUE %in% app_first_winter, TRUE, FALSE))

# Extract features for weekday use only
app_sess_weekday_f <- app_session_first %>%
...
app_sess_weekend_f <- app_session_first %>%
  mutate(time = hms::as_hms(app_open_time)) %>%
  filter(in_firstterm == "firstterm",
...
feat_slope_sessdur <- app_session_first %>%
  filter(in_firstterm == "firstterm") %>%
...
app_session_first_f <- app_session_first %>%
  filter(in_firstterm == "firstterm")
feat_slope_totalsess <- my.slopeExtraction(df= app_session_first_f, date_var = app_open_date, feat_name = "ae_coef_totalsess")
```
...
campus_event_attendance_retention <- join_df_retention(campus_event_attendance, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
campus_event_attendance_first <- mutate_var_in_firstterm(campus_event_attendance_retention, checkin_time) %>% filter(in_firstterm == "firstterm") #directly classify in "firstterm" and "not firstterm" and exclude all logs that lie out of the frist term
```
...
# Create a joint data set containing term of admission
campus_wall_post_retention <- join_df_retention(campus_wall_post, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
campus_wall_post_first <- mutate_var_in_firstterm(campus_wall_post_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
# Create a joint data set containing term of admission
campus_wall_post_like_retention <- join_df_retention(campus_wall_post_like, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
campus_wall_post_like_first <- mutate_var_in_firstterm(campus_wall_post_like_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
# Create a joint data set containing term of admission
campus_wall_comment_retention <- join_df_retention(campus_wall_comment, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
campus_wall_comment_first <- mutate_var_in_firstterm(campus_wall_comment_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
# Create a joint data set containing term of admission
campus_wall_comment_like_retention <- join_df_retention(campus_wall_comment_like, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
campus_wall_comment_like_first <- mutate_var_in_firstterm(campus_wall_comment_like_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
...
# Create a joint data set containing term of admission
social_group_member_retention <- join_df_retention(social_group_member, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_member_first <- mutate_var_in_firstterm(social_group_member_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat memb socgrp}
...
# Create a joint data set containing term of admission
social_group_post_retention <- join_df_retention(social_group_post, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_post_first <- mutate_var_in_firstterm(social_group_post_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat socgrp post}
...
# Create a joint data set containing term of admission
social_group_post_like_retention <- join_df_retention(social_group_post_like, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_post_like_first <- mutate_var_in_firstterm(social_group_post_like_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_comment_first <- mutate_var_in_firstterm(social_group_comment_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
...
#### Likes of comments on posts in social groups
Classification of logs as during or outside of first term:
```{r joint set socgrp post com like}
# Create a joint data set containing term of admission
social_group_comment_like_retention <- join_df_retention(social_group_comment_like, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_comment_like_first <- mutate_var_in_firstterm(social_group_comment_like_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat socgrp post com like}
feat_soc_grp_com_like_f <- social_group_comment_like_first %>%
  group_by(user_id) %>%
  summarize(ce_total_likes_giv_com_post_sg = n()) #group comment likes per person
```

#### Ratio of posts to comments in social groups
Join feat_post und feat_comment by id to get ration of posts to comments
```{r feat ratio socgrp post com} 
soc_grp_ratio_f <- inner_join(feat_soc_grp_post_f, feat_soc_grp_com_f, by= 'user_id')
head(soc_grp_ratio_f)
colSums(is.na(soc_grp_ratio_f))

feat_soc_grp_ratio_f <- soc_grp_ratio_f %>%
  group_by(user_id) %>%
  summarize(ce_ratio_posttocom_sg = ce_total_post_sg/ce_total_com_post_sg) #ratio of posts to comments
```
...
# Create a joint data set containing term of admission
chat_message_retention <- join_df_retention(chat_message, join_by = c("user_id" = "sender_id"))
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
chat_message_first <- mutate_var_in_firstterm(chat_message_retention, date_created) %>% rename("sender_id" = "user_id") %>% filter(in_firstterm == "firstterm")
```
...
feat_slope_mes_sent <- my.slopeExtraction(df = df_for_slope_mes_sent, date_var = date_created, feat_name = "ce_coef_mes_sent")
...
# Create a joint data set containing term of admission
clicked_campus_guide_tab_retention <- join_df_retention(clicked_campus_guide_tab, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_campus_guide_tab_first <- mutate_var_in_firstterm(clicked_campus_guide_tab_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat camp tab}
...
feat_slope_camp_guide <- my.slopeExtraction(df = clicked_campus_guide_tab_first, date_var = Date, feat_name = "ae_coef_click_cg")
```
...
```{r joint set com tab}
# Create a joint data set containing term of admission
clicked_community_tab_retention <- join_df_retention(clicked_community_tab, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_community_tab_first <- mutate_var_in_firstterm(clicked_community_tab_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
feat_slope_com <- my.slopeExtraction(df = clicked_community_tab_first, date_var = Date, feat_name = "ae_coef_click_cmnty")
```
...
# Create a joint data set containing term of admission
clicked_home_tab_retention <- join_df_retention(clicked_home_tab, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_home_tab_first <- mutate_var_in_firstterm(clicked_home_tab_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
```{r joint set not tab}
# Create a joint data set containing term of admission
clicked_notifications_tab_retention <- join_df_retention(clicked_notifications_tab, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_notifications_tab_first <- mutate_var_in_firstterm(clicked_notifications_tab_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
...
feat_slope_not <- my.slopeExtraction(df = clicked_notifications_tab_first, date_var = Date, feat_name = "ae_coef_click_not")
```
...
# Create a joint data set containing term of admission
clicked_profile_tab_retention <- join_df_retention(clicked_profile_tab, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_profile_tab_first <- mutate_var_in_firstterm(clicked_profile_tab_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat prfl tab}
...
feat_slope_prfl <- my.slopeExtraction(df = clicked_profile_tab_first, date_var = Date, feat_name = "ae_coef_click_prfl")
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
clicked_tile_first <- mutate_var_in_firstterm(clicked_tile_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat tiles}
...
feat_slope_tile <- my.slopeExtraction(df = clicked_tile_first, date_var = Date, feat_name = "ae_coef_click_tiles")
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
friend_request_first <- mutate_var_in_firstterm(friend_request_retention, date_created) %>% rename("sender_id" = "user_id") %>% filter(in_firstterm == "firstterm")
```
Preparation:
```{r frnds}
...
Classification of logs as during or outside of first term:
```{r joint set or ev att}
# Create a joint data set containing term of admission
orientation_event_attendance_retention <- join_df_retention(orientation_event_attendance, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
orientation_event_attendance_first <- mutate_var_in_firstterm(orientation_event_attendance_retention, checkin_time) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat or ev att}
feat_or_ev_att_f <- orientation_event_attendance_first %>% 
  replace_with_na_at("feedback_rating", ~.x < 0) %>% # turn negative (-1) feedback ratings to NA, as those were apparently missing
  group_by(user_id) %>% 
  summarize(ce_total_or_ev=n(), #number of orientation events per person
            ce_mean_rat_or_ev = mean(feedback_rating, na.rm = T), #mean rating of events per person
            ce_sd_rat_or_ev = sd(feedback_rating, na.rm = T),
            ce_max_rat_or_ev = my.max(feedback_rating),
            ce_min_rat_or_ev = my.min(feedback_rating))
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
performed_search_first <- mutate_var_in_firstterm(performed_search_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat perf sear}
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
service_attendance_first <- mutate_var_in_firstterm(service_attendance_retention, checkin_time) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat serv att}
...
### Courses students added
Classification of logs as during or outside of first term:
```{r joint set courses}
# Create a joint data set containing term of admission
user_added_course_retention <- join_df_retention(user_added_course, join_by = "user_id")
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
user_added_course_first <- mutate_var_in_firstterm(user_added_course_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat courses}
feat_add_cour_f <- user_added_course_first %>%
  group_by(user_id) %>% 
  summarize(ae_total_cour_add=n())
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
user_campus_event_first <- mutate_var_in_firstterm(user_campus_event_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat camp ev}
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
viewed_campus_event_first <- mutate_var_in_firstterm(viewed_campus_event_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat view camp ev}
...
feat_slope_view_campev <- my.slopeExtraction(df = viewed_campus_event_first, date_var = Date, feat_name = "ae_coef_view_campev")
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
viewed_club_first <- mutate_var_in_firstterm(viewed_club_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat view club}
...
feat_slope_view_club <- my.slopeExtraction(df = viewed_club_first, date_var = Date, feat_name = "ae_coef_view_club")
```
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
viewed_user_profile_first <- mutate_var_in_firstterm(viewed_user_profile_retention, date_created) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat view prfl}
...
feat_slope_actions <- my.slopeExtraction(df= all_engagement, date_var = date, feat_name = "ce_coef_actions")
```
...
```{r}
ev_all_f <- full_join(feat_camp_ev_att_f, feat_or_ev_att_f, by = "user_id")
ev_all_f <- full_join(ev_all_f, feat_serv_att_f, by = "user_id")
...
feat_ev_all_f <- ev_all_f %>%
  group_by(user_id) %>%
  summarize(ce_total_ev = ce_total_camp_ev + ce_total_or_ev + ce_total_serv)
```
...
### Network features
```{r }
stud_retention <- retention_clean %>% select(user_id)
...
feature_matrix <- join_all(list(feat_add_cour_f, feat_all_engagement, feat_all_text, feat_app_sess_f,
                                ...
                                feat_chat_msg_sgcom_f, feat_chat_msg_sgpost_f, feat_chat_msg_wlcom_f, feat_chat_msg_wlpost_f,
                                feat_cl_camp_guide_tab_f, feat_cl_com_tab_f, feat_cl_home_tab_f, feat_cl_not_tab_f,
                                feat_cl_prfl_tab_f, feat_cl_tile_f, feat_com_all_f, feat_comSocGrpMembr_at_event_f,
                                ...
                                feat_entropy_serv_att_f,feat_entropy_soc_grp_com_f,
                                ...
                                feat_or_ev_att_f, feat_perf_search_f, feat_post_com_all_ratio_f, feat_posts_all_f,
                                feat_reg_camp_ev_f, feat_reg_frnd_f, feat_reg_soc_grp_f, feat_ret_f,
                                feat_serv_att_f, feat_soc_grp_com_f, feat_soc_grp_com_like_f, feat_soc_grp_com_text_f,
                                feat_soc_grp_frnd_req_f, feat_soc_grp_memb_f, feat_soc_grp_post_f, feat_soc_grp_post_like_f,
                                feat_soc_grp_post_text_f, feat_soc_grp_ratio_f, feat_total_friends_f, feat_total_messages_f,
...
to_be_replaced = to_be_replaced[!grepl(pattern = "_mean_rat", to_be_replaced)] #remove mean ratings again, those are supposed to be imputed later
...
feature_matrix$institution <- "uni_2"

# save matrix as new csv file
write.csv(feature_matrix,'../../Prep Data/feat_mat_uni_2.csv')
```
