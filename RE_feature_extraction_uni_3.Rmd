---
title: "Retention Project | Feature extraction uni_3"
output:
  html_document:
    code_folding: show
    toc: yes
...
library('knitr')
opts_knit$set(root.dir = normalizePath('../Data/Raw Data/uni_3'))
```
...
udf_firstterm <- function(df, date, term_dates) {
  ...
  feat_df <- df %>% mutate(term_of_admission = str_replace_all(term_of_admission, "[\\[\\]]", "")) %>%
    mutate(in_firstterm = case_when(term_of_admission == "201710" & 
                                      !!date >= term_dates$fall_2017_start & 
                                      !!date <= term_dates$fall_2017_end ~ "firstterm",
                                    term_of_admission == "201810" & 
                                      !!date >= term_dates$fall_2018_start & 
                                      !!date <= term_dates$fall_2018_end ~ "firstterm",
                                    term_of_admission == "201910" & 
                                      !!date >= term_dates$fall_2019_start & 
                                      !!date <= term_dates$fall_2019_end ~ "firstterm",
                                    term_of_admission == "202010" & 
...
udf_app_session_cleaner <- function(df,date, term_dates){
  ...
  df %>% udf_firstterm(date = app_open_time, term_dates = term_dates) %>%
    ...
    mutate(app_before_firstterm = case_when( # create column that categorizes each session according to whether that session was before the respective student's first term start
      term_of_admission == "Fall 2016" & app_open_time < term_dates$fall_2016_start ~ "TRUE", #i.e. if a student was admitted in fall 2016 and used the app before the start of term 2016, he/she gets a TRUE
      term_of_admission == "Fall 2017" & app_open_time < term_dates$fall_2017_start ~ "TRUE",
      term_of_admission == "Fall 2018" & app_open_time < term_dates$fall_2018_start ~ "TRUE",
    ...
    mutate(app_first_winter = case_when( #create column that categorizes each session according to whether that session was during the respective student's first winter break
      term_of_admission == "Fall 2016" & app_open_time > term_dates$fall_2016_end & app_open_time < term_dates$spring_2017_start ~ "TRUE", #i.e. if a student was admitted in fall 2016 and used the app after end of the frist term and before the start of the second term, he/she gets a TRUE
      term_of_admission == "Fall 2017" & app_open_time > term_dates$fall_2017_end & app_open_time < term_dates$spring_2018_start ~ "TRUE",
      term_of_admission == "Fall 2018" & app_open_time > term_dates$fall_2018_end & app_open_time < term_dates$spring_2019_start ~ "TRUE",
      term_of_admission == "Fall 2019" & app_open_time > term_dates$fall_2019_end & app_open_time < term_dates$spring_2020_start ~ "TRUE",
      #term_of_admission == "Fall 2020" & app_open_time > term_dates$fall_2020_end & app_open_time < ymd(20200125) ~ "TRUE",
      TRUE ~ "FALSE"))
}
...
```
```{r}
retention %>% filter(user_id != 0)

retention %>% .$ admit_term %>% str()
...
term_dates <- list(fall_2016_start = ymd(20160820),
               fall_2016_end = ymd(20161220),
               spring_2017_start = ymd(20170125),
               spring_2017_end = NA,
               fall_2017_start = ymd(20170820),
               fall_2017_end = ymd(20171220),
               spring_2018_start = ymd(20180125),
               spring_2018_end = NA,
               fall_2018_start = ymd(20180820),
               fall_2018_end = ymd(20181220),
               spring_2019_start = ymd(20190125),
               spring_2019_end = NA,
               fall_2019_start = ymd(20190820),
               fall_2019_end = ymd(20191220),
               spring_2020_start = ymd(20200125),
               spring_2020_end = NA,
               fall_2020_start = ymd(20200820),
               fall_2020_end = ymd(20201220)
              )
...
retention$user_id %>% unique() %>% length()

retention$user_id %>% length()
```
...
retention <- retention %>% rename(term_of_admission = admit_term,
                     status = enroll_status,
                     high_school_gpa = hs_gpa,
                     pell_eligible = pell,
                     act_score = act, 
                     first_gen_student = `1st_gen`) %>%
  mutate(pell_eligible = dplyr::recode(pell_eligible, N='Not Pell Eligible' , Y='Pell Eligible'),
         status = dplyr::recode(status, E="Continued" , D="Transferred")
         )
...
feats = c('user_id',
          ...
          'high_school_gpa', 
          'gender', 
          'living_in_residence_hall', 'living_in_residence',
          'first_gen_student',
          'athlete', 
          'pell_eligible',
          'ethnicity')
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
social_group_comment_like_first <- udf_firstterm(social_group_comment_like_retention, date_created, term_dates) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
orientation_event_attendance_first <- udf_firstterm(orientation_event_attendance_retention, checkin_time, term_dates) %>% filter(in_firstterm == "firstterm")
'
```
Feature extraction:
```{r feat or ev att}
...
# Add column classifying each row into firstterm and not firstterm and filter for logs from first term
user_added_course_first <- udf_firstterm(user_added_course_retention, date_created, term_dates) %>% filter(in_firstterm == "firstterm")
```
Feature extraction:
```{r feat courses}
...
```{r}
# add column classifying message as between friends or not
...
feature_matrix$institution <-  "uni_3"

# save matrix as new csv file
write.csv(feature_matrix,'../../Prep Data/feat_mat_uni_3.csv')
```
