---
title: "Retention Project | Feature extraction uni_1"
output:
  html_document:
    code_folding: show
    toc: yes
...
library('knitr')
opts_knit$set(root.dir = normalizePath('../Data/Raw Data/uni_1'))
...
retention <- read_csv("user_retention_data.csv")
```
...
mutate_var_in_firstterm <- function(df, date) {
 ...
 feat_df <- df %>%
  mutate(in_firstterm = case_when(term_of_admission == "Fall 2018" & !!date >= ymd(20180820) & !!date <= ymd(20181220) ~ "firstterm",#!! used to unquote an input so that it’s evaluated, not quoted
...
The institutional data looks different for each university. To make it easily comparable and enable a joint analysis with other data, the variable naming and factor coding is aligned with the first university.

```{r}

retention <- retention %>%
  rename("gender" = "gender_a_(legal_sex)") %>%
  mutate(status = dplyr::recode(status,
         ...
         gender = dplyr::recode(gender, 
                                'F' = "Female (F)",
                                'M' = "Male (M)"),
         athlete = dplyr::recode(athlete,
                                 'Y' = "Athlete this Term",
                                 'N' = "Never an Athlete"),
         living_in_residence = dplyr::recode(living_in_residence,
                                             'R' = "Yes",
                                             'C' = "No"),
         program_status = dplyr::recode(program_status,
                                        'F' = "Fulltime",
                                        'P' = "Parttime"),
         in_state = dplyr::recode(in_state,
                                  'Y' = "Yes",
                                  'N' = "No"),
         ethnicity = dplyr::recode(ethnicity,
                                   'White' = "Caucasian/White non-Hispanic (1)",
                                   'American Indian or Alaska Native' = "American Indian/Alaska Native (5)",
                                   'Asian' = "Asian (4)",
                                   'Black or African American' = "African-American (2)",
                                   'Hispanics of any race' = "Hispanic/Other (3)",
                                   'Nonresident Alien' = "Non Resident Alien (7)",
                                   'Two or more races' = "Two or more races (8)"))

retention <- replace_with_na_at(retention,
                 "living_in_residence",
                 condition = ~.x == "W") #we do not know whether they lived in residence or commuted
...
retention_clean <- retention %>% filter(!is.na(term_of_admission)) %>% distinct() %>% filter(user_id != 0)
...
feat_ret_f <- retention_clean %>%
  ...
  summarize(status,
            ...
            inst_gender = gender,
            inst_living_in_residence = living_in_residence,
            inst_athlete = athlete,
            inst_ethnicity = ethnicity,
            inst_spiritual_identity = spiritual_identity,
            inst_in_state = in_state,
            inst_program_status = program_status,
            inst_international_student = international_student)
```
...
app_session_first <- app_session_first %>%
  filter(app_open_time > ymd(20180101),
         app_close_time > ymd(20180101)) %>% # app was only launched in 2018, dates before 2018 are unvalid
  ...
  mutate(app_before_firstterm = case_when( #create column that categorizes each session according to whether that session was before the respective student's first term start
    term_of_admission == "Fall 2018" & app_open_time < ymd(20180820) ~ "TRUE",#i.e. if a student was admitted in fall 2018 and used the app before the start of term 2018, he/she gets a TRUE
  ...
  mutate(app_first_winter = case_when( #create column that categorizes each session according to whether that session was during the respective student's first winter break
    term_of_admission == "Fall 2018" & app_open_time > ymd(20181220) & app_open_time < ymd(20190125) ~ "TRUE",#i.e. if a student was admitted in fall 2018 and used the app after end of the frist term and before the start of the second term, he/she gets a TRUE
...
feat_down_n_reg <- download_and_registration %>% 
  filter(download_time > ymd(20180101), registration_time > ymd(20180101)) %>% 
...
We see that the orientation events only seem to have been registered for the 2019 cohort. Therefore, we do not calculate the features based on `orientation_event_attendance`.
The code below is therefore commented out. Should we at some point get the full data set, we can include it in the feature calculation again.
...
We see that the courses added only seem to have been registered for the 2018 cohort. Therefore, we do not calculate the features based on `user_added_course`.
The code below is therefore commented out. Should we at some point get the full data set, we can include it in the feature calculation again.
...
We had to leave out the orientation events (feat_or_ev_att_f) because the feature could not be calculated.
```{r}
...
feat_ev_all_f <- ev_all_f %>%
  group_by(user_id) %>%
  summarize(ce_total_ev = ce_total_camp_ev + ce_total_serv) # add ce_total_or_ev again once the feat_or_ev_att_f can be calculated
```
...
## with all features for app users
```{r}
...
feature_matrix <- join_all(list(feat_app_sess_f, feat_app_sess_f2, feat_app_sess_week_f, feat_slope_sessdur, feat_slope_totalsess,
                                feat_all_engagement, feat_all_text,
                                feat_camp_ev_att_f, feat_camp_ev_frnd_req_f,
                                ...
                                feat_perf_search_f, feat_post_com_all_ratio_f, feat_posts_all_f,
                                ...
                                feat_slope_actions, feat_slope_mes_sent, 
                                ...
                                feat_net) %>% 
                             map(function(x){mutate(x, user_id = as.double(user_id))}),
...
feature_matrix$institution <- "uni_1"

# save matrix as new csv file
write.csv(feature_matrix,'../../Prep Data/feat_mat_uni_1.csv')

```


```{r}
list(feat_app_sess_f, feat_app_sess_f2, feat_app_sess_week_f, feat_slope_sessdur, feat_slope_totalsess,
                                feat_all_engagement, feat_all_text,
                                feat_camp_ev_att_f, feat_camp_ev_frnd_req_f,
                                feat_camp_ev_type_f, feat_camp_wl_com_f, feat_camp_wl_com_like_f, feat_camp_wl_com_text_f,
                                feat_camp_wl_post_f, feat_camp_wl_post_like_f,feat_camp_wl_post_text_f, feat_chat_message_week_f,
                                ...
                                feat_slope_mes_rec, feat_slope_camp_guide, feat_slope_com, feat_slope_not, 
                                feat_slope_prfl, feat_slope_tile, feat_slope_view_campev, feat_slope_view_club,
                                feat_net) %>% map(function(x){mutate(x, user_id = as.double(user_id))})



feature_matrix %>% tail(50)

union(feat_net$user_id, feat_down_n_reg$user_id) %>% length()

# something is wrong with down_n_reg




feature_matrix %>% filter(is.na(status))

```

```{r}
feat_entropy_chat_msg_rec_f

feat_frnd_msg_f

intersect(feat_entropy_chat_msg_rec_f$user_id, feat_app_sess_f2$user_id) %>% length()
